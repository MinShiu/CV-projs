...
def detected(frame, frame_cpy, cnt, x, y, w, h, cx, cy, sex, currentSendCount):
    ...
    ## push notification to app if more than 200 visitors detected
...
cap = cv2.VideoCapture('rtsp://admin:12345@10.0.33.78:554/MediaInput/h264')
...
persons = [] 
counta = 0    # current count in female toilet
countaL = 0    # total female visitors
countb = 0    # current count in male toilet
countbL = 0    # total male visitors
pid = 1
...
with open(r'C:\Users\admin\Desktop\log_toilet1.txt', 'r') as f:
...
while(cap.isOpened()):
    ...
    for cnt in contours0:
        ...
        if area > AREA_TH :
            ...
            if min_dist < 60:
                ...
                if i.going_LEFT(365) and cy < 250 and time.time()-t1 > 0.75:
                    t1 = time.time()
                    timer = time.time()
                    counta += 1 
                    countaL += 1
                    ...
                    if currentSendCountF >= 200:
                        currentSendCountF = 0
                        
                ## going out from female toilet
                elif i.going_RIGHT(365) and cy < 250 and time.time()-t1 > 0.75:
                    t1 = time.time()
                    timer = time.time()
                ...
                elif i.going_UP(520) and time.time()-t2 > 0.75:
                    t2 = time.time()
                    timer = time.time()
                    if countb > 0:
                        countb -= 1 
                ...
                elif i.going_DOWN(520) and time.time()-t2 > 0.75:
                    t2 = time.time()
                    timer = time.time()
                    countb += 1
                    countbL += 1
        ...
        str_a = 'COUNT A : '+ str(counta)
        str_b = 'COUNT B : ' + str(countb)
        straL = 'IN_A : ' + str(countaL)
        strbL = 'IN_B : ' + str(countbL)
        ...
        cv2.putText(frame, straL ,(40,140), FONT, 0.5, (0,0,255), 1, cv2.LINE_AA)
        cv2.putText(frame, strbL ,(40,170), FONT, 0.5, (0,0,255), 1, cv2.LINE_AA)
    ...
    if (ctime.split(' ')[1].split(':')[1] == '00' or ctime.split(' ')[1].split(':')[1] == '30') and ctime.split(' ')[1].split(':')[2] == '00' and not(writeOnce):
        writeOnce = True
        string = '[' + ctime + '] countA:' + str(counta) + ' countB:' + str(countb) + ' InA:' + str(countaL) + ' InB:' + str(countbL) + '\n'
        with open(r'C:\Users\admin\Desktop\log_toilet1.txt', 'a') as f:
            f.write(string)
