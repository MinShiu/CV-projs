...
parser.add_argument(
    ...
    dest='cap',
    type=str,
    default='rtsp://admin:12345@10.0.33.179:554/MediaInput/h264',
...
parser.add_argument(
    ...
    dest='logfile',
    type=str,
    default=r'C:\Users\admin\Desktop\lvl3lby1.txt',
    help='Log file path')
...
fgbg = cv2.createBackgroundSubtractorKNN(
    history=600, dist2Threshold=700, detectShadows=False)
...
while(cap.isOpened()):
    ...
    if listening_F:
        ...
        if time.time() - tF > 60:
            print('REQUEST: female lby1 do request')
            ltimeF = True
            listening_F = waitForComplete('status_lobby1_female', listening_F, tunnel)
            currentSendCountF = 0
            showCountTimerResetF = time.time()
    ...
    if listening_M:
        ...
        if time.time() - tM > 60:
            print('REQUEST: male lby1 do request')
            ltimeM = True
            listening_M = waitForComplete('status_lobby1_male', listening_M, tunnel)
            currentSendCountM = 0
            showCountTimerResetM = time.time()
    else:
    ...
    if performIncompleteAction_M:
        performIncompleteAction_M = incompleteAction(tunnel, 'lby1m')
        
    if performIncompleteAction_F:
        performIncompleteAction_F = incompleteAction(tunnel, 'lby1f')
    ...
    for cnt in contours0:
        ...
        if area > AREA_TH:
            ...
            if min_dist < 50:
                ...
                if i.going_RIGHT(700) and 50 <= cy < 90 and time.time() - t1 > 0.5:
                    t1 = time.time()
                    timer = time.time()
                    countM += 1
                    countML += 1
                    currentSendCountM += 1
                    i.resetTracks()
                    detected(frame, '1', cnt, x, y, w,
                             h, cx, cy, 'M', currentSendCountM)
                    if currentSendCountM >= 45:
                        currentSendCountM = 0
                ...
                if i.going_RIGHT(690) and cy >= 110 and time.time()-t2 > 0.5:
                    t2 = time.time()
                    timer = time.time()
                    countF += 1
                    countFL += 1
                    ...
                    i.resetTracks()
                    detected(frame, '1', cnt, x, y, w,
                             h, cx, cy, 'F', currentSendCountF)
                    if currentSendCountF >= 30:
                        currentSendCountF = 0
        ...
        cv2.putText(frame, strTF, (40, 250), FONT,
                    0.5, (0, 0, 255), 1, cv2.LINE_AA)
        #cv2.circle(frame, (700,100), 5, (255,0,0), -1)
        #cv2.circle(frame, (700,50), 5, (255,0,0), -1)
    ...
    if time.time() - showCountTimerResetM > 4500:
        requests.get(tunnel + '/incomplete/lby1m')
        str_triggered1 = str(currentSendCountM) + 'users at L3 (Lobby1) male toilet \
    ...
    if time.time() - showCountTimerResetF > 3600:
        requests.get(tunnel + '/incomplete/lby1f')
        str_triggered2 = str(currentSendCountM) + 'users at L3 (Lobby1) female toilet \
            last seen 75 mins ago, attention required. '
